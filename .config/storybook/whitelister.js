...
const whitelistresolver = (path, ignore, depth) => {
  ...
  // useful with [].filter
  const createIgnoreFilter = exclusions => item =>
    exclusions.indexOf(item) === -1;
  ...
  const normalizeArray = arr =>
    (arr || []).reduce(flatten, []).filter(x => x).filter(isUnique);

  const packages = Object.keys(require(path).dependencies || {}).filter(
    createIgnoreFilter(normalizeArray(ignore))
  );

  // grab the denormalized list of child deps (may be nested with dupes)
  const uncleanChildren = packages.map(d =>
    Object.keys(require(`${d}/package.json`).dependencies || {})
  );
  ...
  // ignore list.
  const childDeps = normalizeArray(uncleanChildren).filter(
    createIgnoreFilter(childIgnoreList)
  );
  ...
  if (maxDepth > 0) {
    ...
    const deepIgnoreList = normalizeArray(
      [].concat(ignore).concat(packages).concat(childDeps)
    );
    ...
    const uncleanDescendents = childDeps.map(desc =>
      whitelistresolver(`${desc}/package.json`, deepIgnoreList, maxDepth - 1)
    );

    // flatten, dedupe and remove falseys then remove previously found items or things in original ignore list.
    descendents = normalizeArray(uncleanDescendents).filter(
      createIgnoreFilter(deepIgnoreList)
    );
  }

  return normalizeArray(
    [].concat(packages).concat(childDeps).concat(descendents)
  );
};

module.exports = ignore => {
  const whitelist = whitelistresolver(
    '../../package.json',
    ignore || ['react']
  );
